...
def make_dataset_MyDataset(root: str):
    img_list = []
    for hazy_name in os.listdir(os.path.join(root)):
        img_list.append(os.path.join(root, hazy_name))
    return img_list

def make_dataset_ohaze(root: str, mode: str, iftest: bool):
    img_list = []
    if iftest:
        for idx, img_name in enumerate(os.listdir(os.path.join(root, mode, 'hazy'))):
            if (idx < 35):
                 continue
...
def make_dataset_hazerd(root: str, mode: str):
    img_list = []
    for gt_name in os.listdir(os.path.join(root, 'img')):
        hazy_name = gt_name.replace('RGB', mode)
        assert os.path.exists(os.path.join(root, 'simu', hazy_name))
        img_list.append([os.path.join(root, 'simu', hazy_name),
...
class OHazeDataset(data.Dataset):
    def __init__(self, root, mode, iftest=False):
        self.root = root
        self.mode = mode
        self.imgs = make_dataset_ohaze(root, mode, iftest)
...
class HazeRDDataset(data.Dataset):
    def __init__(self, root, mode):
        ...
        self.imgs = []
        for m in mode:
            self.imgs += make_dataset_hazerd(root, m)
...
class MYDataset(data.Dataset):
    def __init__(self, root):
        self.root = root
        self.imgs = make_dataset_MyDataset(root)
