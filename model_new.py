...
class DM2FNet(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        ...
        self.down1 = nn.Sequential(
            ComplexDownsample(256, num_features), nn.SELU()
        )
        self.down2 = nn.Sequential(
            ComplexDownsample(512, num_features), nn.SELU()
        )
        self.down3 = nn.Sequential(
            ComplexDownsample(1024, num_features), nn.SELU()
        )
        self.down4 = nn.Sequential(
            ComplexDownsample(2048, num_features), nn.SELU()
        )
        ...
        self.attention_phy = nn.Sequential(
            ...
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features * 4, kernel_size=1),
            ChannelAttention(num_features * 4)
        )
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        self.down0 = nn.Sequential(
            ComplexDownsample(64, num_features), nn.SELU(),
            ComplexDownsample(num_features, num_features), nn.SELU(),
            ComplexDownsample(num_features, num_features), nn.SELU(),
        )
        self.down1 = nn.Sequential(
            ComplexDownsample(256, num_features), nn.SELU(),
